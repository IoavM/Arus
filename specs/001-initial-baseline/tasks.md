...
**Purpose**: Project initialization and basic structure setup for Python FastAPI backend and React Vite frontend.

- [x] T001 Create project directory structure for backend and frontend per implementation plan
- [x] T002 Initialize Python FastAPI backend with dependencies (`FastAPI`, `Pydantic`, `SQLAlchemy`, `asyncpg`, `Alembic`, `PyJWT`, `passlib`) in `backend/pyproject.toml`
- [x] T003 [P] Initialize React Vite frontend with dependencies (`react`, `axios`, `react-router-dom`) in `frontend/package.json`
...
**Independent Test**: Register a new company via `POST /api/v1/auth/register-company` and verify tenant creation and owner access.
...
- [x] T017 [US1] Add unit test for company registration & tenant creation in `backend/tests/unit/test_register_company.py`
...
**Independent Test**: Login via `POST /api/v1/auth/login` with valid/invalid credentials and verify JWT claims.
...
- [x] T021 [US2] Add unit test for login authentication & JWT token claims in `backend/tests/unit/test_auth.py`
...
- [x] T026 [US3] Add unit test for RBAC route protection (Admin allowed, Seller forbidden) in `backend/tests/unit/test_rbac.py`
...
- [x] T031 [US4] Add unit test for product soft-delete and stock validation in `backend/tests/unit/test_products.py`
...
- [x] T036 [US5] Add unit test for customer creation and default customer retrieval in `backend/tests/unit/test_customers.py`
...
- [x] T041 [US8] Add unit test for atomic stock addition on purchase in `backend/tests/unit/test_purchases.py`
...
- [x] T046 [US6] Add unit test for atomic sale transaction, row locking, and negative stock rejection in `backend/tests/unit/test_sales.py`
...
**Independent Test**: Fetch sales history list and verify receipt line items and totals match.
...
- [x] T049 [US7] Add integration test for sales history querying in `backend/tests/integration/test_sales_history.py`
...
- [x] T052 [US9] Add unit test for inventory stock calculation from movements in `backend/tests/unit/test_inventory.py`
...
- [x] T055 [US10] Add integration test for executive metrics aggregation & Admin-only RBAC protection in `backend/tests/integration/test_dashboard.py`

---

## Phase 13: Polish & Cross-Cutting Concerns

**Purpose**: Multi-tenant security hardening, migrations, and end-to-end scenario validation.

- [x] T056 [P] Implement multi-tenant isolation integration test suite in `backend/tests/integration/test_tenant_isolation.py`
- [x] T057 [P] Execute database migration scripts via Alembic in `backend/alembic/versions/`
- [x] T058 Execute quickstart validation guide scenarios per `quickstart.md`
- [x] T059 [P] Update documentation and API contract references in `docs/`
...
- All Foundational tasks marked `[P]` (T006, T007, T008, T010, T011) can run in parallel.
- Once Foundational phase completes, frontend UI views `[P]` and backend models `[P]` for different user stories can be worked on in parallel by team members.

---

## Implementation Strategy

### MVP First (Step 1 & Step 2)
1. Complete Setup + Foundational phases.
2. Complete US1 (Register Company) + US2 (Login).
3. Complete US4 (Products) + US5 (Customers) + US6 (Sales).
4. **STOP and VALIDATE**: Verify end-to-end sales transactions and multi-tenant isolation.

### Full Baseline Delivery
1. Complete remaining stories (US3 Users, US8 Purchases, US7 Sales History, US9 Inventory, US10 Dashboard Summary).
2. Run full test suite in `pytest` and `quickstart.md` validation.
